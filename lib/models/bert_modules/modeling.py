...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, hidden_states_other, attention_mask, output_attention_probs=False):
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, input_tensor_other, attention_mask, output_attention_probs=False):
        self_output = self.self(input_tensor, input_tensor_other, attention_mask, output_attention_probs=output_attention_probs)
        if output_attention_probs:
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, hidden_states_other, attention_mask, output_attention_probs=False):
        attention_output = self.attention_text(hidden_states, hidden_states_other, attention_mask, output_attention_probs=output_attention_probs)
        attention_output_other = self.attention_visual(hidden_states_other, hidden_states, attention_mask, output_attention_probs=output_attention_probs)
        if output_attention_probs:
        ...
        if output_attention_probs:
            return layer_output, layer_output_other, attention_probs, attention_probs_other
        else:
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, hidden_states_other, attention_mask, output_all_encoded_layers=False, output_attention_probs=False):
        all_encoder_layers = []
        all_attention_probs = []
        for layer_module in self.layer:
            layer_out = layer_module(hidden_states, hidden_states_other, attention_mask, output_attention_probs=output_attention_probs)
            if output_attention_probs:
                hidden_states, hidden_states_other, attention_probs, attention_probs_other = layer_out
